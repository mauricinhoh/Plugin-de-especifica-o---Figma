...
export function App() {
  ...
  // Necessário porque o listener de mensagens é registrado uma única
  // vez (useEffect com deps vazias, logo abaixo) — sem isso,
  // `handleMainMessage` ficaria "travado" enxergando sempre o valor
  // de `screen` do momento em que o plugin abriu (mesmo padrão já
  // usado em itemsRef acima).
  ...
  function handleDownloadDocx() {
    const result = downloadHandoffDocx(usageLog, designerName);
    if (!result.ok && result.errorMessage) {
      setErrorBanner(result.errorMessage);
    }
  }
    ...
    <div className="app-shell">
      ...
      {screen === "step0" && (
        <Step0
          ...
          onClose={handleClose}
          hasUsageHistory={usageLog.length > 0}
