...
function buildComponentTypeOptions(): ComponentTypeOption[] {
  ...
  // 12 categorias fixas exigidas pela especificação de Handoff — a
...
async function generateSpecifications(items: SpecificationItem[]): Promise<void> {
  try {
    ...
    // O total do card "Ordem de leitura" é calculado dentro de
    // generatePanel, a partir de TODOS os itens presentes na
    // especificação no momento da geração (automáticos + adicionados
    // manualmente), excluindo os do tipo Decorativo (Regra 2 — não
    // recebem número de ordem de leitura, mas continuam com
    // card/marcação normalmente).
    const panel = await generatePanel(screenNode as SceneNode, ordered);
    ...
    const logEntry: UsageLogEntry = {
      ...
      items: ordered.map((item) => ({
        ...
        links: findRuleByKey(item.ruleKey)?.links
      }))
    };
...
figma.ui.onmessage = (message: UiToMainMessage) => {
  switch (message.type) {
    case "ui-ready":
      ...
      safely("histórico de uso da página", () => postToUi({ type: "usage-log", entries: getUsageLog() }));
      break;
